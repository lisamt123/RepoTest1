...
if(StaticData.DisabledTriggers!= null && StaticData.DisabledTriggers.contains('Opportunity')){
...
return;
    ...
    if(trigger.isBefore && trigger.isInsert) {
        ...
        system.debug('Bazinga.... ' + trigger.new.size());
        //system.debug('Bazinga.... ' + trigger.newMap.keyset().size());
        
        handler_Opportunity.beforeInsert(trigger.new);
    }
    
    //Before Update
    if(trigger.isBefore && trigger.isUpdate) {
        
        handler_Opportunity.beforeUpdate(trigger.new, trigger.newMap.keyset(), trigger.oldMap);
    }
    
    //BEFORE DELETE
    if(trigger.isBefore && trigger.isDelete) {
        
        handler_Opportunity.beforeDelete(trigger.old);
    }
    
    // AFTER INSERT
    if(trigger.isAfter && trigger.isInsert) {
        
        handler_Opportunity.afterInsert(trigger.new);
    }
    
    // AFTER UPDATE
    if(trigger.isAfter && trigger.isUpdate) {   
        
        handler_Opportunity.afterUpdate(trigger.new, trigger.oldMap);
    }
